...
def main(page: ft.Page):
    ...
    # Çözünürlük Menüsü (İlk başta gizli)
    cozunurluk_dropdown = ft.Dropdown(
        label="Çözünürlük",
        width=340,
        visible=False,
        options=[
            ft.dropdown.Option("En Yüksek"),
    ...
    # Format Değiştiğinde Çalışacak Fonksiyon
    def format_ayarla(e):
        nonlocal format_secimi
        format_secimi = format_dropdown.value
        if format_secimi == "MP4 (Video)":
            cozunurluk_dropdown.visible = True
        else:
            cozunurluk_dropdown.visible = False
        page.update()
    ...
    format_dropdown = ft.Dropdown(
        ...
        width=340,
        options=[
            ft.dropdown.Option("MP3 (Ses)"),
            ft.dropdown.Option("MP4 (Video)"),
        ],
        value="MP3 (Ses)",
        on_change=format_ayarla
    )

    # İndirme Butonu Tetikleyicisi
    def indir_buton_click(e):
        ...
        # Telefonda indirilenler klasörünü hedef alıyoruz
    ...
    indir_butonu = ft.ElevatedButton(
        text="İNDİRMEYİ BAŞLAT",
        width=200,
        height=50,
        color=ft.colors.WHITE,
        bgcolor=ft.colors.BLUE_700,
        on_click=indir_buton_click
    )
